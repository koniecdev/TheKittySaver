...
public sealed class ApartmentNumber : ValueObject
{
    ...
    {
        var result = Result.Create(value, DomainErrors.PolishAddressEntity.ApartmentNumberProperty.NullOrEmpty)
            .Ensure(v => !string.IsNullOrWhiteSpace(v), DomainErrors.PolishAddressEntity.ApartmentNumberProperty.NullOrEmpty)
            .Ensure(v => v.Length <= MaxLength, DomainErrors.PolishAddressEntity.ApartmentNumberProperty.LongerThanAllowed)
